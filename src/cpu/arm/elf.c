...
int loadELF(char* args[], int argCount, FIL *file){
	...
	if(header.h_data == 1){
		puts_debug("ELF file is little endian");
	} else if (header.h_data == 2){
		puts_debug("ELF file is big endian");
	} else {
		puts("[?] ELF file has invalid endianness");
		return -1;
	}
	...
	switch (header.type) {
		...
		case ET_NONE:
			...
			return -1;
		case ET_REL:
			puts_debug("ELF is relocatable");
			break;
		case ET_EXEC:
			puts_debug("ELF is executable");
			break;
		case ET_DYN:
